...
*/
Web2Cal.icalStore=new Array();
Web2Cal.readIcal=function(path, callback, opts )
{ 
	var arr=new Array();
	if(WUtil.isString(path))
	{
		arr.push(path);
	}
	else
	...
	{
		var path=arr[i];
		var ptemp=path.replace(/\//g, "");
		var ptemp=ptemp.replace(".", ""); 
		Web2Cal.icalStore[ptemp]=undefined;
		if (Web2Cal.icalStore[ptemp]!=undefined) 
		{
			var calendarObj=	Web2Cal.icalStore[ptemp];
		...
		else 
		{
			jQuery.get(path, [],
				function (content)
				{  
						var icalParser=new Web2Cal.icalParser();
						...
						Web2Cal.icalStore[ptemp]=calendarObj;
						icalGroups.push(calendarObj); 
						if(icalGroups.length==arr.length)
						{
					 		callback(icalGroups);
...
Web2Cal.icalParser.stripTime=function(utc)
{
	var t= ( utc.getTime() - ((utc.getHours()*60*60*1000) + ( utc.getMinutes() *60*1000 )+(utc.getSeconds()*1000) ));
	return t;
}
  // Class methods
          ...
          // It's an event. Add property and value to event.
		   ...
		   //Escape some characters. This is a temporary fix.	
			...
			if (convertIcalKey(key) == "endTime") 
			{
				...
				if((( Web2Cal.icalParser.stripTime(ed)-Web2Cal.icalParser.stripTime(st) )/864e5)  > 1)
				{
					this.calendar.getWeb2CalEvent(this.eventCount).allDay=true;
				}
				this.calendar.getWeb2CalEvent(this.eventCount)["resizable"]=true;
				this.calendar.getWeb2CalEvent(this.eventCount)["movable"]=true;
			}
